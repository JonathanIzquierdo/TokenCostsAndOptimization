...
Antes de discutir las cuatro capas hay una decisión binaria, anterior, que define a cuáles capas tenés acceso. Esta distinción suele ser tácita y produce expectativas equivocadas: alguien lee este documento, ve "95% de ahorro con caching y batch", y se frustra cuando descubre que su consumo es de Claude.ai o Copilot — donde esas palancas simplemente no existen para el usuario final.

**Si consumís IA como producto cerrado** (Claude.ai, ChatGPT, Copilot, Cursor):
- Tenés acceso a Capa 2 (settings de IDE) y a una versión limitada de Capa 3 (configuración del producto, como elegir el modelo o ajustar telemetría).
- **No tenés acceso a Capa 1**: no podés tocar caching, thinking, max_tokens, batch, ni ningún parámetro de la llamada. Esas decisiones las toma el producto por vos.
- **No tenés acceso a Capa 4**: el routing lo decide el proveedor (lo más cercano que tenés es el Auto Mode de Copilot).
- Techo total realista de ahorro: **alrededor del 30% sobre el consumo no optimizado**. Más allá de eso requiere otra arquitectura.
...
- Techo total realista de ahorro: **más del 90% sobre el consumo no optimizado**.

**Este documento asume que estás construyendo, o yendo hacia ahí.** Las capas que siguen dan por sentado que tu equipo escribe código de aplicación contra una API o está evaluando hacerlo. Si hoy estás 100% en producto cerrado y querés bajar costos significativos, el paso anterior es construir (al menos) un wrapper interno o una app delgada que use API keys directas; recién a partir de ahí abren las palancas que valen la pena.
...
Es transparente para el flujo del agente — la lógica no cambia, simplemente se le ahorra tener que procesar (y pagar) tokens basura. Activar siempre.
...
El bloque `cache_control: {"type": "ephemeral"}` marca explícitamente cuáles partes del prompt se cachean. La métrica `cache_read_input_tokens` en la respuesta te dice cuántos tokens vinieron del cache (al 10%) vs cuántos se procesaron normales — es la forma de verificar que el caching efectivamente está funcionando.
...
Fuente: [Finout, 2026](https://www.finout.io/blog/anthropic-api-pricing)
...
Nota la elección de Haiku para el resumen: usar el modelo flagship para resumir sería tirar plata. Resumir es una tarea simple donde Haiku rinde igual a 1/3 del precio.
...
Es elegible para ZDR (Zero Data Retention), así que no agrega complicaciones de compliance. Si tu equipo está construyendo agentes complejos, vale la pena evaluarla antes de invertir en lógica custom de compactación.
...
3. **Observabilidad y alertas** — Para que el costo no se desborde sin avisar.
...
El patrón mínimo viable es un **budget monitor** que rastree gasto por usuario/equipo y alerte al 50%, 80% y 100% del presupuesto mensual.
...
Lo más útil de este bloque es la última variable: `OTEL_RESOURCE_ATTRIBUTES`. Permite etiquetar los datos con `team.id` y `department`, así en el dashboard podés filtrar y comparar consumo entre equipos. Cuando se aplica vía MDM enterprise, esta variable se setea automáticamente según el equipo al que pertenece el dev — el reporting queda preciso sin requerir que cada dev configure nada.

### 9.2 OTel en Copilot Chat

Para Copilot la activación es por settings, más simple aún:
...
Una vez levantado, abrir http://localhost:18888 — vas a ver trazas en tiempo real de cada request que haga Claude Code o Copilot. Es el camino más rápido para que un dev pruebe localmente "qué se ve" en observabilidad de IA antes de invertir en infraestructura para producción.
